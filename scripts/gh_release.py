...
APK_PATH = r"D:/WorkBuddy_Save/手机小程序开发/MetroNext-release-1.0.32.apk"
ASSET_NAME = "MetroNext-v%s-release.apk" % VER

BODY = (
    "## 更新内容（v1.0.32）\n"
    "- 修复：收藏站点较少时，4×2 / 4×4 列表行距被拉开、中间大片留白的问题——\n"
    "  空位不再被拉伸，每行高度恒定，内容贴顶显示\n"
    "- 修复：2×2 小部件窄屏下文字被截断（如「18:00 · 1…」）——班次行改为\n"
    "  「时刻 / 倒计时」左右两列，可用宽度 78 → 90dp\n"
    "- 新增：长文案自动适配——站名、线路方向、下一站过长时自动换行或缩小字号，\n"
    "  不再直接截断（优先级：换行 > 缩小字号 > 省略号）\n"
    "- 优化：2×2 站名字号 14 → 13sp；小部件选择器预览图同步更新\n"
    "- 说明：不同手机、不同收藏数量下，小部件展示的信息量完全一致，\n"
    "  仅根据格子大小调整排版\n\n"
    "## 更新内容（v1.0.31，未单独发版，一并包含）\n"
    "- 修复：全新安装后小部件一直显示「暂无收藏」的空态问题\n"
    "  （此前版本在特定路径下会发出不被识别的旧版数据快照）\n\n"
    "## 说明\n"
